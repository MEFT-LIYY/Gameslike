...
function buildCoverHTML(src, altText, options = {}) {
  if (!src || /<img|<picture/i.test(src)) {
    return String(src).replace(
      /<img([^>]*?)>/i,
      (m, attrs) => `<img loading="lazy" decoding="async" ${attrs}>`
    );
  }
  const alt = ESC(altText || '');
  const webp = src.replace(/\.(jpg|jpeg|png)$/i, '.webp');
  const isWebp = /\.webp$/i.test(src);
  const ratioStyle = 'aspect-ratio:16/9;width:100%;height:auto;display:block;';
  if (isWebp) {
    return `<img src="${src}" alt="${alt}" loading="lazy" decoding="async" style="${ratioStyle}" referrerpolicy="no-referrer">`;
  }
  return `
    <picture>
      <source srcset="${webp}" type="image/webp">
      <img src="${src}" alt="${alt}" loading="lazy" decoding="async" style="${ratioStyle}" referrerpolicy="no-referrer">
    </picture>`;
}
...
function render(list){
    ...
    grid.innerHTML = pageList.map(g=>{
      ...
      const coverUrl = (g.cover || '').trim();
      return `<article class="card">
        <div class="thumb">${buildCoverHTML(coverUrl, title)}${corner}${newest}</div>
